...
Shader "SineVFX/TranslucentCrystals/Crystal"
{
	Properties
	{
		...
		[Space][Header(Translucency)]
		[HDR]_TransluencyTint("Transluency Tint", Color) = (1,1,1,1)
		_TDistortion( "Distortion", Float ) = 1
		_TScale( "Scale", Float ) = 1
		_TAttenuation( "Attenuation", Float ) = 1
		_TAmbient( "Ambient", Float ) = 1
	...
	SubShader
	{
		...
		float3 SubsurfaceShadingSimple(float3 diffColor, float3 normal, float3 viewDir, float3 thickness, UnityLight light)
		{
			...
			half  fLTDot = pow(saturate(dot(viewDir, -vLTLight)), 1) * _TScale;
			half3 fLT = _TAttenuation * (fLTDot + _TAmbient) * (thickness);
			return diffColor * ((light.color * fLT)) * _TransluencyTint;
		}
		...
		inline half4 LightingStandardCustom(SurfaceOutputStandardCustom s, half3 viewDir, UnityGI gi )
		{
			...
			//half4 c = half4( s.Albedo, 0);// * translucency * _Translucency, 0 );

			half3 c = SubsurfaceShadingSimple(s.Albedo, s.Normal, viewDir, s.Translucency, gi.light);
			...
			r.Alpha = s.Alpha;
			return LightingStandard (r, viewDir, gi) + float4(c, 0);
		}
		...
		void surf( Input i , inout SurfaceOutputStandardCustom o )
		{
			...
			#else
				float4 staticSwitch20 = ( tex2DNode5 * _EmissionColor * _EmissionPower * lerpResult17 );
			#endif
			...
			float3 temp_cast_2 = tex2D( _TranslucencyMask, uv_TranslucencyMask );
